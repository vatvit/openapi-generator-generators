...
public class LaravelMaxGenerator extends AbstractPhpCodegen implements CodegenConfig {
  ...
  /**
   * Write Resource files using Mustache templates
  ...
  private void writeResourceFiles() {
    for (Map<String, Object> task : resourceGenerationTasks) {
      ...
      try {
        // Add namespace configuration to template data
        data.put("resourceNamespace", resourceNamespace);
        data.put("resourceFinal", resourceFinal);

        // Add flag for mixed type detection
        String baseType = (String) data.get("baseType");
        data.put("isMixed", baseType == null || "mixed".equals(baseType));

        // Determine which template to use
        String templatePath;
        if (isCollection != null && isCollection) {
          templatePath = "laravel-max/resource-collection.mustache";
        } else {
          templatePath = "laravel-max/resource.mustache";
        }
  ...
  // NOTE: generateResourceContent() removed - now using resource.mustache template
  // See GENDE-019 for extraction details

  /**
   * Write Controller files using Mustache template (one file per operation)
   */
  private void writeControllerFiles() {
    for (Map<String, Object> task : controllerGenerationTasks) {
      ...
      try {
        // Add namespace configuration to template data
        data.put("controllerNamespace", controllerNamespace);
        data.put("handlerNamespace", handlerNamespace);
        data.put("formRequestNamespace", formRequestNamespace);
        data.put("controllerFinal", controllerFinal);

        // Add flags for template conditionals
        String summary = (String) data.get("summary");
        String notes = (String) data.get("notes");
        data.put("hasSummary", summary != null && !summary.isEmpty());
        data.put("hasNotes", notes != null && !notes.isEmpty());

        // Load and process template
        String templatePath = "laravel-max/controller.mustache";
        InputStream templateStream = this.getClass().getClassLoader().getResourceAsStream(templatePath);
  ...
  // NOTE: generateControllerContent() removed - now using controller.mustache template
  // See GENDE-019 for extraction details

  /**
   * Write FormRequest files using Mustache template
   */
  private void writeFormRequestFiles() {
    for (Map<String, Object> task : formRequestGenerationTasks) {
      ...
      try {
        // Add namespace configuration to template data
        data.put("formRequestNamespace", formRequestNamespace);
        data.put("formRequestFinal", formRequestFinal);

        // Precompute validation rules for template
        ...
        if (vars != null) {
          for (CodegenProperty var : vars) {
            ...
            Map<String, Object> ruleEntry = new HashMap<>();
            ruleEntry.put("fieldName", var.baseName);
            ruleEntry.put("rules", rules);
            validationRules.add(ruleEntry);
          }
        }
        data.put("validationRules", validationRules);

        // Load and process template
        String templatePath = "laravel-max/form-request.mustache";
        InputStream templateStream = this.getClass().getClassLoader().getResourceAsStream(templatePath);
  ...
  // NOTE: generateFormRequestContent() removed - now using form-request.mustache template
  // See GENDE-019 for extraction details
